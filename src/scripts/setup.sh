...
# Check if the 'exampledb' database exists and create it if not
DB_EXISTS=$(docker exec "$CONTAINER_ID" psql -U postgres -tAc "SELECT 1 FROM pg_database WHERE datname = 'exampledb'")
if [ "$DB_EXISTS" != "1" ]; then
  docker exec "$CONTAINER_ID" psql -U postgres -c "CREATE DATABASE exampledb WITH ENCODING='UTF8' LC_COLLATE='en_US.utf8' LC_CTYPE='en_US.utf8';"
  echo "Database 'exampledb' created."
else
  echo "Database 'exampledb' already exists."
fi

######## REDIS #######
echo "[REDIS] Initiating setups for redis..."
...
docker run \
  ...
  -v redis_volume:/data \
  --rm --name redis\
  -d redis:latest || { echo "❗️Redis Container failed to start"; exit 1; }
